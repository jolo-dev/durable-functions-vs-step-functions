import{SESClient as c,SendEmailCommand as p}from"@aws-sdk/client-ses";import{DynamoDBClient as u,PutItemCommand as f}from"@aws-sdk/client-dynamodb";var w=new c({}),E=new u({}),r=async o=>{console.log("send-email request",o);let{email:e,taskToken:i}=o;if(!e)throw new Error("Missing required fields: email");let a=new Date().toISOString(),s="Demo: Your Verification Code",t=Math.floor(1e5+Math.random()*9e5).toString(),d=`Here is your Code: ${t}`,l=`
    <html>
  ...
  `;await E.send(new f({TableName:process.env.TABLE_NAME,Item:{id:{S:e},sort:{S:t},status:{S:"pending"},timestamp:{S:a}}}));let m=new p({Source:process.env.SENDER_EMAIL,Destination:{ToAddresses:[e]},Message:{Subject:{Data:s},Body:{Text:{Data:d},Html:{Data:l}}}});try{return await w.send(m),{timestamp:a,taskToken:i,code:t,status:"sent"}}catch(n){throw new Error(`Failed to send email to ${e}: ${n instanceof Error?n.message:"Unknown error"}`)}};var b=async(o,e)=>{await e.waitForCallback(async a=>await r({email:o.email,taskToken:a}))==="APPROVED"?await e.step(async()=>await performAction(todo)):await e.step(async()=>await recordRejected(todo,o.approverEmail))};export{b as handler};
