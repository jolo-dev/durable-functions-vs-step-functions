import{SESClient as c,SendEmailCommand as p}from"@aws-sdk/client-ses";import{DynamoDBClient as u,PutItemCommand as f}from"@aws-sdk/client-dynamodb";var y=new c({}),E=new u({}),r=async t=>{console.log("send-email request",t);let{email:e,taskToken:n}=t;if(!e||!n)throw new Error("Missing required fields: email or taskToken");let i=new Date().toISOString(),a="Demo: Your Verification Code",o=Math.floor(1e5+Math.random()*9e5).toString(),d=`Here is your Code: ${o}`,m=`
  ...
  `;await E.send(new f({TableName:process.env.TABLE_NAME,Item:{id:{S:e},sort:{S:o},status:{S:"pending"},timestamp:{S:i}}}));let l=new p({Source:process.env.SENDER_EMAIL,Destination:{ToAddresses:[e]},Message:{Subject:{Data:a},Body:{Text:{Data:d},Html:{Data:m}}}});try{return await y.send(l),{timestamp:i,taskToken:n,code:o,status:"sent"}}catch(s){throw new Error(`Failed to send email to ${e}: ${s instanceof Error?s.message:"Unknown error"}`)}};var S=async(t,e)=>{let{email:n}=t;await r(n)};export{S as handler};
